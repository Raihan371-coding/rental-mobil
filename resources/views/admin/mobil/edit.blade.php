...
@section('content')
    ...
    <div class="mb-6">
        <div class="flex items-center gap-2 text-sm text-slate-500 mb-3">
            ...
            <span class="text-slate-900 font-medium">Edit Mobil</span>
        </div>
        <div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
            <div>
                <h1 class="text-2xl font-bold text-slate-900">Edit Mobil</h1>
                <p class="mt-1 text-sm text-slate-500">Perbarui data kendaraan <span
                        class="font-semibold text-slate-700">{{ $mobil->nama_mobil }}</span>.</p>
            </div>
    ...
    <div class="rounded-2xl bg-white shadow-sm border border-slate-100 p-5 sm:p-8">
        <div class="flex items-center gap-3 border-b border-slate-100 bg-slate-50">
            ...
            <div>
                <h2 class="text-base font-bold text-slate-900">Form Edit Mobil</h2>
                <p class="text-xs text-slate-400">Kode Mobil: <span
                        class="font-mono font-semibold text-slate-600">{{ $data->kode_mobil ?? '-' }}</span></p>
            </div>
        </div>
                ...
                <div>
                    ...
                    <select name="status"
                        ...
                        <option value="tersedia" {{ old('status', $mobil->status) == 'tersedia' ? 'selected' : '' }}>
                            Tersedia</option>
                        <option value="disewa" {{ old('status', $mobil->status) == 'disewa' ? 'selected' : '' }}>Disewa
                        </option>
                        <option value="service" {{ old('status', $mobil->status) == 'service' ? 'selected' : '' }}>Service
                        </option>
                    </select>
                    @error('status')
                        <p class="mt-1 text-xs text-red-500">{{ $message }}</p>
                ...
                <div class="sm:col-span-2">
                    ...
                    @if ($mobil->foto)
                        <div class="mb-3 flex items-center gap-4">
                            <img src="{{ asset('storage/' . $mobil->foto) }}" alt="Foto Mobil"
                                class="h-24 w-36 rounded-xl border border-slate-200 object-cover shadow-sm">
                    ...
                    <p class="mt-1.5 text-xs text-slate-400">Kosongkan jika tidak ingin mengganti foto.</p>
                    @error('foto')
                        <p class="mt-1 text-xs text-red-500">{{ $message }}</p>
                    @enderror
                </div>
