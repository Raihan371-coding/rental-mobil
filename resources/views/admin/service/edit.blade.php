...
@section('content')
    ...
    <div class="mb-6">
        <div class="flex items-center gap-2 text-sm text-slate-500 mb-3">
            ...
            <span class="text-slate-900 font-medium">Edit Service</span>
        </div>
        <div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
            <div>
                <h1 class="text-2xl font-bold text-slate-900">Edit Service</h1>
                <p class="mt-1 text-sm text-slate-500">Perbarui data service <span
                        class="font-semibold text-slate-700">{{ $service->kode_service ?? '-' }}</span>.</p>
            </div>
    ...
    <div class="rounded-2xl bg-white shadow-sm border border-slate-100 overflow-hidden">
        <div class="flex items-center gap-3 border-b border-slate-100 bg-slate-50 px-6 py-4">
            ...
            <div>
                <h2 class="text-base font-bold text-slate-900">Form Edit Service</h2>
                <p class="text-xs text-slate-400">Kode Service: <span
                        class="font-mono font-semibold text-slate-600">{{ $service->kode_service ?? '-' }}</span></p>
            </div>
        </div>

        <form action="{{ route('admin.service.update', $service->id) }}" method="POST" class="space-y-6 p-6">
            @csrf
            @method('PUT')
            ...
            <div class="grid gap-5 sm:grid-cols-2">
                    ...
                    <select name="mobil_id"
                        ...
                        @foreach ($mobils as $mobil)
                            <option value="{{ $mobil->id }}"
                                {{ old('mobil_id', $service->mobil_id) == $mobil->id ? 'selected' : '' }}>
                ...
                <div>
                    ...
                    <div class="relative">
                        ...
                        <input type="number" name="biaya_service"
                            value="{{ old('biaya_service', $service->biaya_service) }}"
                            class="block w-full rounded-xl border border-slate-200 bg-slate-50 pl-10 pr-4 py-2.5 text-sm text-slate-900 focus:border-cyan-500 focus:bg-white focus:outline-none focus:ring-2 focus:ring-cyan-100 transition">
                    </div>
                    @error('biaya_service')
                        <p class="mt-1 text-xs text-red-500">{{ $message }}</p>
                    @enderror
                ...
                <div>
                    ...
                    <select name="status_service"
                        ...
                        <option value="pending"
                            {{ old('status_service', $service->status_service) == 'pending' ? 'selected' : '' }}>Pending
                        </option>
                        <option value="proses"
                            {{ old('status_service', $service->status_service) == 'proses' ? 'selected' : '' }}>Proses
                        </option>
                        <option value="selesai"
                            {{ old('status_service', $service->status_service) == 'selesai' ? 'selected' : '' }}>Selesai
                        </option>
                    </select>
                ...
                <div class="sm:col-span-2">
                    ...
                    <textarea name="deskripsi" rows="4"
                        class="block w-full rounded-xl border border-slate-200 bg-slate-50 px-4 py-2.5 text-sm text-slate-900 focus:border-cyan-500 focus:bg-white focus:outline-none focus:ring-2 focus:ring-cyan-100 transition">{{ old('deskripsi', $service->deskripsi) }}</textarea>
                    @error('deskripsi')
                        <p class="mt-1 text-xs text-red-500">{{ $message }}</p>
                    @enderror
                </div>
