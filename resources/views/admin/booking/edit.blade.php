...
@section('content')
    ...
    <div class="mb-6">
        <div class="flex items-center gap-2 text-sm text-slate-500 mb-3">
            ...
            <span class="text-slate-900 font-medium">Edit Booking</span>
        </div>
        <div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
            <div>
                <h1 class="text-2xl font-bold text-slate-900">Edit Booking</h1>
                <p class="mt-1 text-sm text-slate-500">Perbarui data pemesanan pelanggan #{{ $booking->id }}.</p>
            </div>
            <div class="flex items-center gap-2">
                @include('admin.booking.partials.status-badge', ['status' => $booking->status])
                <a href="{{ route('admin.booking.index') }}"
                    class="inline-flex items-center justify-center gap-2 rounded-xl border border-slate-200 bg-white px-4 py-2.5 text-sm font-semibold text-slate-600 shadow-sm hover:bg-slate-50 transition-colors">
    ...
    <form action="{{ route('admin.booking.update', $booking->id) }}" method="POST" id="bookingForm" class="space-y-6">
        @csrf
        @method('PUT')
            ...
            <div class="relative grid gap-5 sm:grid-cols-2">
                ...
                <label class="block text-sm text-slate-700">
                    ...
                    <input type="text" name="nama_pelanggan"
                        value="{{ old('nama_pelanggan', $booking->nama_pelanggan) }}" placeholder="Nama lengkap pelanggan"
                        class="mt-2 block w-full rounded-xl border border-slate-200 bg-slate-50 px-4 py-3 text-sm text-slate-900 focus:border-sky-500 focus:bg-white focus:outline-none focus:ring-2 focus:ring-sky-100 transition-colors">
        ...
        <div class="relative overflow-hidden rounded-2xl bg-white p-6 shadow-sm border border-slate-100">
            ...
            <div class="relative grid gap-5 sm:grid-cols-2 xl:grid-cols-3">
                ...
                <label class="block text-sm text-slate-700">
                    ...
                    <select name="status"
                        ...
                        <option value="menunggu konfirmasi"
                            {{ old('status', $booking->status) == 'menunggu konfirmasi' ? 'selected' : '' }}>Menunggu
                            Konfirmasi</option>
                        <option value="terkonfirmasi"
                            {{ old('status', $booking->status) == 'terkonfirmasi' ? 'selected' : '' }}>
                ...
                <label class="block text-sm text-slate-700">
                    ...
                    <input type="time" name="jam_booking" value="{{ old('jam_booking', $booking->jam_booking) }}"
                        class="mt-2 block w-full rounded-xl border border-slate-200 bg-slate-50 px-4 py-3 text-sm text-slate-900 focus:border-sky-500 focus:bg-white focus:outline-none focus:ring-2 focus:ring-sky-100 transition-colors">
                </label>
            ...
            <label class="relative mt-5 block text-sm text-slate-700">
                ...
                <textarea name="keterangan" rows="4" placeholder="Catatan tambahan (opsional)"
                    class="mt-2 block w-full rounded-xl border border-slate-200 bg-slate-50 px-4 py-3 text-sm text-slate-900 focus:border-sky-500 focus:bg-white focus:outline-none focus:ring-2 focus:ring-sky-100 transition-colors">{{ old('keterangan', $booking->keterangan) }}</textarea>
            </label>
        </div>
