        ...
        <div class="min-h-screen lg:flex">
            <aside class="border-b border-slate-200 bg-white lg:min-h-screen lg:w-80 lg:border-r">
                <div class="mx-auto flex max-w-7xl flex-col gap-8 px-6 py-8 sm:px-8 lg:max-w-none">
                    ...
                    <nav class="space-y-2">
                        <a href="{{ route('pengembalian.index') }}" class="block rounded-3xl px-4 py-3 text-sm font-semibold transition {{ request()->routeIs('pengembalian.*') ? 'bg-sky-100 text-sky-700' : 'text-slate-700 hover:bg-slate-50' }}">Pengembalian</a>
                        <a href="{{ route('pembayaran.index') }}" class="block rounded-3xl px-4 py-3 text-sm font-semibold transition {{ request()->routeIs('pembayaran.*') ? 'bg-sky-100 text-sky-700' : 'text-slate-700 hover:bg-slate-50' }}">Pembayaran</a>
                        <a href="{{ route('rental.index') }}" class="block rounded-3xl px-4 py-3 text-sm font-semibold transition {{ request()->routeIs('rental.*') ? 'bg-sky-100 text-sky-700' : 'text-slate-700 hover:bg-slate-50' }}">Rental</a>
                    </nav>
