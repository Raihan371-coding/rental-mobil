    ...
    <form action="{{ route('rental.store') }}" method="POST" class="space-y-6">
        ...
        <div class="grid gap-6 sm:grid-cols-2">
            ...
            <div>
                ...
                <input type="text" id="id_customer" name="id_customer" value="{{ old('id_customer') }}" class="mt-2 block w-full rounded-lg border border-slate-300 px-3 py-2 text-sm shadow-sm focus:border-sky-500 focus:outline-none focus:ring-1 focus:ring-sky-500">
            </div>
            <div>
                <label for="id_mobil" class="block text-sm font-semibold text-slate-700">ID Mobil</label>
                <input type="text" id="id_mobil" name="id_mobil" value="{{ old('id_mobil') }}" class="mt-2 block w-full rounded-lg border border-slate-300 px-3 py-2 text-sm shadow-sm focus:border-sky-500 focus:outline-none focus:ring-1 focus:ring-sky-500">
            </div>
            <div>
                <label for="tanggal_rental" class="block text-sm font-semibold text-slate-700">Tanggal Rental</label>
                <input type="date" id="tanggal_rental" name="tanggal_rental" value="{{ old('tanggal_rental') }}" class="mt-2 block w-full rounded-lg border border-slate-300 px-3 py-2 text-sm shadow-sm focus:border-sky-500 focus:outline-none focus:ring-1 focus:ring-sky-500">
            </div>
            <div>
                <label for="tanggal_kembali" class="block text-sm font-semibold text-slate-700">Tanggal Kembali</label>
                <input type="date" id="tanggal_kembali" name="tanggal_kembali" value="{{ old('tanggal_kembali') }}" class="mt-2 block w-full rounded-lg border border-slate-300 px-3 py-2 text-sm shadow-sm focus:border-sky-500 focus:outline-none focus:ring-1 focus:ring-sky-500">
            </div>
            <div>
                <label for="total_harga" class="block text-sm font-semibold text-slate-700">Total Harga</label>
                <input type="number" step="0.01" id="total_harga" name="total_harga" value="{{ old('total_harga') }}" class="mt-2 block w-full rounded-lg border border-slate-300 px-3 py-2 text-sm shadow-sm focus:border-sky-500 focus:outline-none focus:ring-1 focus:ring-sky-500">
            </div>
        ...
        <div class="flex justify-end gap-3">
            ...
            <button type="submit" class="rounded-full bg-violet-600 px-5 py-2 text-sm font-semibold text-white transition hover:bg-violet-700">Simpan</button>
        </div>
    </form>
</div>
@endsection
