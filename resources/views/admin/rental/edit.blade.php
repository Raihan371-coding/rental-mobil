...
@section('content')
    ...
    <div class="mb-6">
        <div class="flex items-center gap-2 text-sm text-slate-500 mb-3">
            ...
            <span class="text-slate-900 font-medium">Edit Rental</span>
        </div>
        <div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
            <div>
                <h1 class="text-2xl font-bold text-slate-900">Edit Rental</h1>
                <p class="mt-1 text-sm text-slate-500">Ubah data kontrak rental <span
                        class="font-semibold text-slate-700">{{ $data->kode_rental }}</span>.</p>
            </div>
    ...
    <div class="rounded-2xl bg-white shadow-sm border border-slate-100 overflow-hidden">
        <div class="border-b border-slate-100 px-6 py-4">
            <div class="flex items-center gap-3">
                <div
                    class="flex h-10 w-10 flex-shrink-0 items-center justify-center rounded-xl bg-amber-100 text-amber-600 font-mono text-xs font-bold">
                    RT
                </div>
                <div>
                    <h3 class="text-base font-bold text-slate-900">{{ $data->kode_rental }}</h3>
                    <p class="text-xs text-slate-400">Perbarui data rental di bawah ini</p>
                </div>
            </div>
        </div>

        <form action="{{ route('admin.rental.update', $data->id) }}" method="POST" class="p-6 space-y-6" id="rentalForm">
            @csrf
            @method('PUT')
            <div class="grid gap-6 sm:grid-cols-2">
                <div>
                    <label for="kode_rental" class="block text-sm font-semibold text-slate-700">Kode Rental</label>
                    <input type="text" id="kode_rental" value="{{ $data->kode_rental }}" readonly
                        class="mt-2 block w-full rounded-xl border border-slate-200 bg-slate-100 px-3.5 py-2.5 text-sm text-slate-600">
                </div>
                <div>
                ...
                <div class="sm:col-span-2">
                    ...
                    <select id="status" name="status"
                        class="mt-2 block w-full rounded-xl border border-slate-200 px-3.5 py-2.5 text-sm shadow-sm focus:border-amber-500 focus:outline-none focus:ring-1 focus:ring-amber-500 transition-colors">
                        <option value="rental" {{ old('status', $data->status) == 'rental' ? 'selected' : '' }}>Rental
                        </option>
                        <option value="kembali" {{ old('status', $data->status) == 'kembali' ? 'selected' : '' }}>Kembali
                        </option>
                    </select>
                </div>
            </div>
