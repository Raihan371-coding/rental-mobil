    ...
    <form action="{{ route('denda.update', $denda->id) }}" method="POST" class="space-y-6">
        ...
        <div class="grid gap-6 sm:grid-cols-2">
            <div>
                ...
                <input type="number" id="rental_id" name="rental_id" value="{{ old('rental_id', $denda->rental_id) }}" class="mt-2 block w-full rounded-lg border border-slate-300 px-3 py-2 text-sm shadow-sm focus:border-sky-500 focus:outline-none focus:ring-1 focus:ring-sky-500">
            </div>
            <div>
