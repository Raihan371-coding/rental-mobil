...
<div class="rounded-4xl bg-white p-8 shadow-xl">
    <div class="mb-8 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        ...
        <a href="{{ route('admin.pengembalian.create') }}" class="inline-flex items-center rounded-full bg-slate-950 px-5 py-3 text-sm font-semibold text-white transition hover:bg-slate-800">Tambah Pengembalian</a>
    </div>
            ...
            <tbody class="divide-y divide-slate-200">
                ...
                <tr class="bg-white hover:bg-slate-50">
                    ...
                    <td class="px-4 py-4 space-x-2">
                        <a href="{{ route('admin.pengembalian.edit', $items->id) }}" class="inline-flex rounded-full bg-slate-950 px-3 py-1.5 text-xs font-semibold text-white transition hover:bg-slate-800">Edit</a>
                        <form action="{{ route('admin.pengembalian.destroy', $items->id) }}" method="POST" class="inline">
                            @csrf
                            @method('DELETE')
