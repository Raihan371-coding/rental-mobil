        ...
        <div class="rounded-lg bg-white p-6 shadow-sm">
            <div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                ...
                <a href="{{ route('customer.booking.create') }}"
                    class="inline-flex items-center rounded-full bg-slate-950 px-5 py-3 text-sm font-semibold text-white transition hover:bg-slate-800">Buat Booking Baru</a>
            </div>
        </div>
                    ...
                    <tbody class="divide-y divide-slate-200 bg-white">
                        @forelse($bookings as $booking)
                            <tr>
                                ...
                                <td class="px-4 py-3 font-medium">{{ $booking->mobil?->nama_mobil ?? 'Tidak ada mobil' }}</td>
                                <td class="px-4 py-3">{{ $booking->tanggal_booking->format('d M Y') }} {{ $booking->jam_booking }}</td>
                                <td class="px-4 py-3">
                                ...
                                <td class="px-4 py-3">
                                    @if($booking->status === 'menunggu konfirmasi')
                                        <div class="flex items-center gap-2">
                                            ...
                                            <form action="{{ route('customer.booking.destroy', $booking) }}" method="POST" class="inline">
                                                @csrf
                                                @method('DELETE')
                                                <button type="submit"
                        ...
                        @empty
                            <tr>
                                <td colspan="6" class="px-4 py-8 text-center text-sm text-slate-500">
                                    Belum ada booking. <a href="{{ route('customer.mobil.index') }}" class="text-sky-600 hover:underline">Lihat katalog mobil</a> untuk mulai booking.
                                </td>
                            </tr>
                        @endforelse
                    </tbody>
