...
<div class="rounded-4xl bg-white p-8 shadow-xl">
    <div class="mb-8 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        ...
        <a href="{{ route('admin.mobil.create') }}" class="inline-flex items-center rounded-full bg-slate-950 px-5 py-3 text-sm font-semibold text-white transition hover:bg-slate-800">
            Tambah Mobil
        </a>
    </div>
                ...
                @forelse ($mobils as $mobil)
                    <tr class="bg-white hover:bg-slate-50">
                        ...
                        <td class="px-4 py-4 space-x-2">
                            ...
                            <form action="{{ route('admin.mobil.destroy', $mobil->id) }}" method="POST" class="inline">
                                @csrf
                                @method('DELETE')
