...
<div class="rounded-4xl bg-white p-8 shadow-xl">
    <div class="mb-8 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        ...
        <a href="{{ route('admin.denda.create') }}" class="inline-flex items-center rounded-full bg-slate-950 px-5 py-3 text-sm font-semibold text-white transition hover:bg-slate-800">Tambah Denda</a>
    </div>
        ...
        <table class="min-w-full divide-y divide-slate-200 text-left text-sm text-slate-700">
            <thead class="border-b border-slate-200 bg-white text-slate-900">
                <tr>
                    ...
                    <th class="px-4 py-3">Kode Denda</th>
            ...
            <tbody class="divide-y divide-slate-200">
                ...
                <tr class="bg-white hover:bg-slate-50">
                    ...
                    <td class="px-4 py-4">{{ $denda->kode_denda}}</td>
                    <td class="px-4 py-4">{{ $denda->rental->kode_rental ?? '-' }}</td>
                    <td class="px-4 py-4">{{ $denda->jumlah_denda }}</td>
                    <td class="px-4 py-4 space-x-2">
                        <a href="{{ route('admin.denda.edit', $denda->id) }}" class="inline-flex rounded-full bg-slate-950 px-3 py-1.5 text-xs font-semibold text-white transition hover:bg-slate-800">Edit</a>
                        <form action="{{ route('admin.denda.destroy', $denda->id) }}" method="POST" class="inline">
                            @csrf
                            @method('DELETE')
