<nav class="fixed top-0 left-0 z-50 w-full bg-white border-b ">
    <div class="mx-auto px-4 sm:px-6 lg:px-8">
        <div class="flex justify-between h-16 items-center ">
            ...
            <div class=" hidden md:flex items-center space-x-3">
                @auth
                    ...
                    @if (($user->role ?? null) === 'admin' || ($user->is_admin ?? false))
                        <a href="{{ route('admin.dashboard') }}"
                            class="px-3 py-2 rounded-md text-sm font-medium text-gray-700 hover:bg-gray-100">Dashboard</a>
                        <form method="POST" action="{{ route('logout') }}">@csrf<button type="submit"
                                class="px-3 py-2 rounded-md text-sm font-medium text-gray-700 hover:bg-gray-100">Logout</button>
                        </form>
                    @else
                        <div class="relative">
                            <button id="user-dropdown-button" type="button"
                                class="px-3 py-2 rounded-md text-sm font-medium text-gray-700 hover:bg-gray-100 inline-flex items-center"
                ...
                @else
                    <a href="{{ route('login') }}"
                        class="px-3 py-2 rounded-md text-sm font-medium text-gray-700 hover:bg-gray-100">Login</a>
                    <a href="{{ route('register') }}"
                        class="px-3 py-2 bg-blue-600 text-white rounded-md text-sm font-medium hover:bg-blue-700">Get
                        Started</a>
                @endauth
            </div>
            <div class="md:hidden">
                <button id="mobile-menu-button" type="button"
                    class="inline-flex items-center justify-center p-2 rounded-md text-gray-700 hover:text-blue-600 hover:bg-gray-100 focus:outline-none transition-colors duration-200"
    ...
    <div id="mobile-menu"
        class="hidden border-t bg-white md:hidden overflow-hidden transition-all duration-300 ease-in-out">
        <div class="px-4 py-3 space-y-2">
            ...
            <div class="border-t-2 border-t-blue-500 pt-2 mt-2 space-y-2">
                @auth
                    ...
                    @if (($user->role ?? null) === 'admin' || ($user->is_admin ?? false))
                        <a href="{{ route('admin.dashboard') }}"
                            class="block rounded-md px-2 py-2 text-sm font-medium text-gray-700 hover:bg-gray-100">Dashboard</a>
                        <form method="POST" action="{{ route('logout') }}">@csrf<button type="submit"
                                class="w-full text-left rounded-md px-2 py-2 text-sm font-medium text-gray-700 hover:bg-gray-100">Logout</button>
                        </form>
                    @else
                        <a href="{{ route('customer.profile') }}"
                            class="block rounded-md px-2 py-2 text-sm font-medium text-gray-700 hover:bg-gray-100">Profile</a>
                        <a href="{{ route('customer.dashboard') }}"
                            class="block rounded-md px-2 py-2 text-sm font-medium text-gray-700 hover:bg-gray-100">Dashboard</a>
                        <form method="POST" action="{{ route('logout') }}">@csrf<button type="submit"
                                class="w-full text-left rounded-md px-2 py-2 text-sm font-medium text-gray-700 hover:bg-gray-100">Logout</button>
                ...
                @else
                    <a href="{{ route('login') }}"
                        class="block rounded-md px-2 py-2 text-sm font-medium text-gray-700 hover:bg-gray-100">Login</a>
                    <a href="{{ route('register') }}"
                        class="block rounded-md px-2 py-2 text-sm font-medium bg-blue-600 text-white hover:bg-blue-700">Get
                        Started</a>
                @endauth
            </div>
        </div>
